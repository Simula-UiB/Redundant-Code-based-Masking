...
OUTPUT_PATH = "first_order_boolean.csv"
...
def make_shares(sbox_out, mask_1):
    mask_2 = sbox_out ^ mask_1
    return [HAMMING_WEIGTH_TABLE[mask_2], HAMMING_WEIGTH_TABLE[mask_1]]
...
mean_values = [[make_shares(sbox_out, mask_1)
                for mask_1 in range(256)] for sbox_out in range(256)]

mean_values_amount = [
    [0 for mean_01 in range(81)] for sbox_out in range(256)]
mean_values_flat = []

for sbox_out in range(256):
    for mask_1 in range(256):
        means = mean_values[sbox_out][mask_1]
        for mean in means:
            mean_values_flat.append(mean)
        mean_values_amount[sbox_out][9*means[0] + means[1]] += 1
...
cov_matrix = cov_matrix = [[0 for x in range(2)]
                           for y in range(2)]
for i in range(2):
...
for experiment in range(N_EXPERIMENTS):
    ...
    for t in range(N_TRACES):
        ...
        idx = (SBOX_TABLE[secret ^ plaintext]*256 + mask_1)*2
        ...
        trace = rd_state.multivariate_normal(
            mean_values_flat[idx:idx+2], cov_matrix, 1)[0]
